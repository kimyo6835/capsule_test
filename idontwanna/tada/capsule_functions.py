...
def class_capsules(inputs, num_classes, iterations, batch_size, name):
    ...
    with tf.variable_scope(name) as scope:
        with tf.variable_scope('votes') as scope:
            ...
            tf.logging.info("{name} votes shape: {votes.get_shape()}")
            ...
            tf.logging.info("{name} votes shape with coord addition: {votes.get_shape()}")
...
def conv_capsule(inputs, shape, strides, iterations, batch_size, name, padding):
    ...
    with tf.variable_scope(name) as scope:
        ...
        inputs_poses = kernel_tile(inputs_poses, shape[0], stride,
                                   padding)  # (?, 14, 14, 32, 4, 4) -> (?, 6, 6, 3x3=9, 32x16=512)

        # Tile the activations needed for the EM routing
        inputs_activations = kernel_tile(inputs_activations, shape[1], stride,
                                         padding)  # (?, 14, 14, 32) -> (?, 6, 6, 9, 32)
        spatial_size = int(inputs_activations.get_shape()[1])  # 6

        # Reshape it for later operations
        inputs_poses = tf.reshape(inputs_poses, shape=[-1, shape[0] * shape[1] * i_size, 16])  # (?, 9x32=288, 16)
        inputs_activations = tf.reshape(inputs_activations, shape=[-1, spatial_size, spatial_size,
                                                                   shape[0] * shape[1] * i_size])  # (?, 6, 6, 9x32=288)
        ...
        return (poses, activations), beta_v, beta_a



def kernel_tile(input, kernel, stride, padding):
    """This constructs a primary capsule layer using regular convolution layer.
