...
from tada import wires_functions_tmp
...
x_train, y_train = wires_functions_tmp.gen_wires_tmp2(n_data=1000, length=20, imsize=32)
x_test, y_test = wires_functions_tmp.gen_wires_tmp2(n_data=1000, length=20, imsize=32)
...
ReLU_conv1 = capsule_functions.conv2d(x, 5, 16, 2, 'SAME', 'ReLU_conv1', True, tf.nn.relu)
...
ConvCaps1, a, b = capsule_functions.conv_capsule(PrimaryCaps, (3, 3, 16, 1), (1, 1, 1, 1), 3, batch_size, "ConvCaps0", 'SAME')
#ConvCaps1 = capsule_functions.class_capsules(ConvCaps0, 1, iterations=3, batch_size=batch_size, name='class_capsules')

y = tf.placeholder(shape=[batch_size, int(imsize/2), int(imsize/2), 17], dtype=tf.float32, name="y")

# ConvCaps1[0]: [4, 32, 32, 1, 4, 4]
# y_final_0: [4, 32, 32, 1, 16] # pose label
y_final_0 = tf.reshape(ConvCaps1[0], [batch_size, int(imsize/2), int(imsize/2), -1, 16], name="y_final_0")
